    ...
    <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 lg:grid-cols-7 gap-4 w-full">
      {data.map((day, index) => (
        <motion.div
          ...
          className={`flex flex-col items-center p-4 rounded-3xl cursor-pointer transition-all duration-500 theme-slow border backdrop-blur-sm shadow-sm dark:shadow-[inset_0_1px_0_0_rgba(255,255,255,0.02)] ${getMoodColor(day.mood)}`}
        >
